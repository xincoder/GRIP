...
max_num_object = 70 # maximum number of observed objects is 70
...
def process_data(pra_now_dict, pra_start_ind, pra_end_ind, pra_observed_last):
	...
	for frame_ind in range(pra_start_ind, pra_end_ind):	
		...
		now_frame_feature = np.array([now_frame_feature_dict.get(vis_id, np.zeros(total_feature_dimension)) for vis_id in visible_object_id_list])
